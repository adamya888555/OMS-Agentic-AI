    ...
    with st.chat_message(msg["role"]):
        if msg["role"] == "user":
            ...
            if isinstance(content, dict) and content.get("order_data"):
                ...
                # Insert dashed line after Order Details but before Overall Status
                ...
                # Display parts before Overall Status + dashed line
                st.text(new_response)

                # Display Overall Status box with improved colors
                st.markdown(
                    f"""
                    <div style="
                ...
                # Display Detailed System Statuses text part below the box
                if detailed_status_text:
                    st.text(detailed_status_text)

            else:
                # For small talk or other messages, just display the response raw
                # but apply the same formatting if Overall Status line found
                response_text = content["response"]
